...
describe('Page: /1000-birds', () => {
  ...
  it('page loads successfully, with all the options', async () => {
    ...
    const response = await global.__SERVER__.inject(options)
    expect(response.statusCode).toBe(200)
    expect(response.payload).toContain('Do you keep at least 1,000 pullets on your farm currently?')
    expect(response.payload).toContain('Yes')
    expect(response.payload).toContain('No')
  })
  ...
  it('no option selected -> show error message', async () => {
    ...
    const postResponse = await global.__SERVER__.inject(postOptions)
    expect(postResponse.statusCode).toBe(200)
    expect(postResponse.payload).toContain('Select yes if you currently have at least 1,000 pullets on your farm')
  })
  it('user selects ineligible option `No` -> display ineligible page', async () => {
    const postOptions = {
      method: 'POST',
    ...
    const postResponse = await global.__SERVER__.inject(postOptions)
    expect(postResponse.payload).toContain('You cannot apply for a grant from this scheme')
    expect(postResponse.payload).toContain('You must keep at least 1,000 pullets on your farm currently.')
    expect(postResponse.payload).toContain('See other grants you may be eligible for.')
  })
