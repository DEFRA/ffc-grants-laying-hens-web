...
describe('Page: /refurbishing-insulation', () => {
  ...
  it('page loads successfully, with all the options - hen', async () => {
    ...
    expect(response.payload).toContain('Will the hen housing have full wall and roof insulation?')
  ...
  it('page loads successfully, with all the options - pullet', async () => {
    ...
    const response = await global.__SERVER__.inject(options)
    expect(response.statusCode).toBe(200)
    expect(response.payload).toContain('Will the pullet housing have full wall and roof insulation?')
    expect(response.payload).toContain('Yes')
    expect(response.payload).toContain('No')
  })
